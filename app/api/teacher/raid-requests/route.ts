...
export async function GET(req: NextRequest) {
  ...
  const raidS = (cls.settings as { raid?: { round?: number } })?.raid ?? {};
  const round = Math.max(0, Number(raidS.round ?? 0));
  ...
  let total = 0;
  let winCount = 0;
  (students ?? []).forEach((st: { game_state?: { raidReq?: { pid?: number; round?: number }; raidWin?: number } }) => {
    const rq = st.game_state?.raidReq;
    if (rq && Number(rq.round ?? -1) === round && Number(rq.pid) >= 1) {
      tally[Number(rq.pid)] = (tally[Number(rq.pid)] ?? 0) + 1;
      total++;
    }
    if (Number(st.game_state?.raidWin ?? -1) === round) winCount++;
  });
  ...
  return NextResponse.json({ round, total, list, winCount, studentTotal: students?.length ?? 0 });
}
