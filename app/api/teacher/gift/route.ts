...
export async function POST(req: NextRequest) {
  ...
  // 단일(student_id) 또는 다중(student_ids[]) 모두 지원
  ...
  if (ids.length === 0) return jsonError(400, "학생을 선택해주세요.");
  ...
  const { data: students } = await supa
    ...
    .in("id", ids)
    .eq("class_id", cls.id);
  if (!students || students.length === 0) return jsonError(404, "학생을 찾을 수 없어요.");
  ...
  for (const st of students) {
    ...
    const { error } = await supa
      .from("students")
      .update({ points: st.points + points, inventory })
      .eq("id", st.id);
    if (!error) done++;
  }
  if (done === 0) return jsonError(500, "지급에 실패했어요.");

  return NextResponse.json({
    ok: true,
    count: done,
    nickname: students.length === 1 ? students[0].nickname : `${done}명`,
