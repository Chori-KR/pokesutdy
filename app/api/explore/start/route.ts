...
export async function POST(req: NextRequest) {
  ...
  const { exploreLimit, rareRate, specialRate, legendRate, shinyRate } = await getClassSettings(supa, student.class_id);
  ...
  const wild = pickWild(rareRate, specialRate, legendRate, BIOMES[biome].types);
