...
export async function POST(req: NextRequest) {
  ...
  const { exploreLimit, rareRate, specialRate, legendRate, shinyRate, gens } = await getClassSettings(supa, student.class_id);
  ...
  const wild = pickWild(rareRate, specialRate, legendRate, BIOMES[biome].types, gens);
